        ...
        try {
          ...
          if (Objects.nonNull(oldImportList)) {
            final var sortedImportDefinitions = oldImportList.getImportDefinitionList()
                .stream()
                .sorted(Comparator.comparing(luaImportDefinition -> luaImportDefinition.getClassNameWithPath().getUnquotedText()))
                .collect(Collectors.toList());
            oldImportList.replace(LuaElementFactory.createImportListElement(file.getProject(), sortedImportDefinitions));
            for (var i = 0; i < oldImportList.getImportDefinitionList().size(); i++) {
              if (!sortedImportDefinitions.get(i).getText().equals(oldImportList.getImportDefinitionList().get(i).getText())) {
                sorted = true;
                break;
              }
            }
