...
public class LuaIdNameReference extends PsiReferenceBase<LuaNamedElement> implements PsiPolyVariantReference {
    ...
    if (myElement instanceof LuaIdName) {
      if (isFirstChildOfVarId() || myElement.getParent() instanceof LuaParentName ||
        ...
        if (((LuaIdName) myElement).isLocal()) {
          ...
          return LuaGVarIndex.INSTANCE.get(key, myElement.getProject(), GlobalSearchScope.allScope(myElement.getProject()))
              .stream()
              .filter(luaIdName -> myElement.getText().equals(luaIdName.getText()) &&
                  ((luaIdName.getParent() instanceof LuaVarId && luaIdName.getParent().getParent() instanceof LuaVarAssignment &&
                      luaIdName.getParent().getChildren().length == 1) ||
                      (luaIdName.getParent() instanceof LuaFuncDefName && luaIdName.getParent().getChildren().length == 1)))
        ...
        if (Objects.nonNull(luaClass)) {
          ...
          var result = elements.stream()
              .filter(classElementDefinition -> (classElementDefinition.getFirstChild() instanceof LuaClassVarDefinition &&
                  ...
                  (classElementDefinition.getFirstChild() instanceof LuaClassMethodDefinition && myElement.getText()
                      .equals(((LuaClassMethodDefinition) classElementDefinition.getFirstChild()).getClassMethodName())))
              .map(PsiElementResolveResult::new)
              .toArray(ResolveResult[]::new);
          if (result.length > 0) {
            return result;
    ...
    if (Objects.nonNull(parentHeader)) {
      ...
      while (Objects.nonNull(parentClassDefinition)) {
        ...
        var result = parentElements.stream()
            .filter(classElementDefinition -> (classElementDefinition.getFirstChild() instanceof LuaClassVarDefinition &&
                myElement.getText().equals(((LuaClassVarDefinition) classElementDefinition.getFirstChild()).getClassVarName())) ||
                (classElementDefinition.getFirstChild() instanceof LuaClassMethodDefinition && myElement.getText()
                    .equals(((LuaClassMethodDefinition) classElementDefinition.getFirstChild()).getClassMethodName())))
            .map(PsiElementResolveResult::new)
            .toArray(ResolveResult[]::new);
      ...
      if (parent instanceof LuaBody) {
        ...
        if (Objects.isNull(result) || result.length == 0) {
          if (Objects.nonNull(((LuaForStatement) parent).getForCondition())) {
            if (myElement.getFirstChild().getText().equals(((LuaForStatement) parent).getForCondition().getIdName().getText())) {
              result = new ResolveResult[]{new PsiElementResolveResult(((LuaForStatement) parent).getForCondition().getIdName())
              };
            }
  ...
  private LuaNamedElement findInParents(LuaNamedElement element, List<LuaClass> parentsClasses) {
    return parentsClasses.stream()
        .map(parentClass -> PsiTreeUtil.findChildrenOfAnyType(parentClass, LuaClassMethodName.class, LuaClassVarName.class)
            .stream()
