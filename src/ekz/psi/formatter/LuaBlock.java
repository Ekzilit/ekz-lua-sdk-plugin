...
public class LuaBlock extends AbstractBlock {
	private static final List<IElementType> ELEMENTS_WITH_INDENT = Arrays.asList(LuaTypes.CLASS_ELEMENT_DEFINITION,
			...
			LuaTypes.FOR_STATEMENT, LuaTypes.VAR_ASSIGNMENT, LuaTypes.RETURN_CONDITION, LuaTypes.BREAK,
			LuaTypes.LOCAL_VAR_ASSIGNMENT, TokenType.WHITE_SPACE, LuaTypes.DO_BLOCK);
	...
	private void createBlocks(List<Block> blocks, ASTNode child) {
		while (child != null) {
			if (!FormatterUtil.containsWhiteSpacesOnly(child) && child.getTextLength() > 0) {
				...
				Block block = new LuaBlock(child, Wrap.createWrap(WrapType.NONE, false), alignment, getSpacingBuilder(child),
						settings);
				blocks.add(block);
			}
	...
	private boolean shouldNotHasAlignment(ASTNode child) {
		return (child.getElementType() == LuaTypes.DO_BLOCK &&
				(child.getTreeParent().getElementType() == LuaTypes.FOR_STATEMENT ||
						child.getTreeParent().getElementType() == LuaTypes.WHILE_STATEMENT)) ||
				child.getElementType() == LuaTypes.CLASS_BODY || child.getElementType() == LuaTypes.TABLE ||
				child.getElementType() == LuaTypes.FUNC_ASSIGNMENT || child.getElementType() == LuaTypes.VALUE ||
				child.getElementType() == LuaTypes.VALUES || child.getElementType() == LuaTypes.FUNC_PARAMS ||
				child.getTreeParent().getPsi() instanceof LuaFile;
	}
	...
	private boolean isElementWithIndent() {
		return ELEMENTS_WITH_INDENT.contains(myNode.getElementType()) ||
				(isNotVarIdInVarAssignment() && isNotVarIdInTableElement());
	}
