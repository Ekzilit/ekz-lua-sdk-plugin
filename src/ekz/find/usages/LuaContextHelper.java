	...
	public static List<LuaBeanDefinition> getBeansByClassName(final LuaClassName className) {
		...
		if (fileList.size() == 1) {
			...
			return PsiTreeUtil.findChildrenOfType(contextFile, LuaClassNameWithPath.class)
					.stream()
					.filter(luaClassNameWithPath -> getClassNameFromFullPath(luaClassNameWithPath.getUnquotedText()).equals(
							className.getUnquotedText()) &&
		...
		if (fileList.size() == 1) {
			...
			if (result.isEmpty()) {
				...
				while (Objects.nonNull(file)) {
					...
					if (result.isEmpty()) {
						final var contextParentNames = PsiTreeUtil.findChildrenOfType(contextParentFile,
								LuaContextParentName.class).toArray();
