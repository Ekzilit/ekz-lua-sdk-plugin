...
public class LuaGotoSuperHandler extends GotoTargetHandler implements PresentableCodeInsightActionHandler {
...
/*        final PsiElement element = getElement(file, editor.getCaretModel().getOffset());
	...
	@Override
	protected @NonNls String getFeatureUsedKey() {
		return "tst feature used key";
	}
	...
	@Override
	protected @NotNull
	@NlsContexts.HintText String getNotFoundMessage(@NotNull Project project, @NotNull Editor editor, @NotNull PsiFile file) {
		return "Super class not found";
	}
		...
		while (Objects.nonNull(parentClass)) {
			var parentElement = parentClass.getClassBody()
					...
					.filter(classElementDefinition -> PsiTreeUtil.findChildOfAnyType(classElement, LuaClassVarName.class,
							...
							.getText()
							.equals(PsiTreeUtil.findChildOfAnyType(classElementDefinition, LuaClassVarName.class,
									LuaClassMethodName.class).getText()))
					.findFirst()
					.orElse(null);
			if (Objects.nonNull(parentElement)) {
				var descriptor = PsiNavigationSupport.getInstance()
						.createNavigatable(project, parentElement.getContainingFile().getVirtualFile(),
								parentElement.getTextOffset());
