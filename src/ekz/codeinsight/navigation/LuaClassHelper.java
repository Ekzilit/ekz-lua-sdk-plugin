...
public class LuaClassHelper {
  ...
  public static boolean isClassContainsClassElement(@NotNull PsiElement element, @NotNull LuaClass luaClass) {
    return luaClass.getClassBody()
        ...
        .anyMatch(classElementDefinition -> element.getText()
            .equals(PsiTreeUtil.findChildOfAnyType(classElementDefinition, LuaClassMethodName.class, LuaClassVarName.class)
                .getText()));
  }
  ...
  public static boolean isDirectChildOfObject(@NotNull final LuaClassHeader classHeader, final LuaClassHeader childClassHeader) {
    return Objects.isNull(childClassHeader.getParentName()) && classHeader.getClassName().getUnquotedText().equals("Object") &&
        !childClassHeader.isEquivalentTo(classHeader);
  }
  ...
  public static boolean hasImportWithClass(@NotNull final LuaClassHeader classHeader, final LuaClassHeader targetClassHeader) {
    final var importDefinitions = PsiTreeUtil.getChildrenOfType(targetClassHeader.getContainingFile(), LuaImportDefinition.class);
    return Objects.nonNull(importDefinitions) && Arrays.stream(importDefinitions)
        .anyMatch(importDefinition -> isImportOfClass(classHeader, importDefinition.getClassNameWithPath().getUnquotedText()));
  }

  private static boolean isImportOfClass(@NotNull final LuaClassHeader classHeader, final String classNameWithPath) {
    return isImportWithSameClassName(classHeader, classNameWithPath) && isImportWithSameClassPath(classHeader, classNameWithPath);
  }
  ...
  public static String getClassPath(final LuaClassName className) {
    var classPackageDefinitions = PsiTreeUtil.getChildrenOfType(className.getContainingFile(), LuaClassPackageDefinition.class);
  ...
  public static Set<String> getElementsNames(final LuaClass luaClass) {
    return getElements(luaClass).stream()
        .filter(
            element -> Objects.nonNull(PsiTreeUtil.findChildOfAnyType(element, LuaClassMethodName.class, LuaClassVarName.class)))
        .map(elementDefinition -> {
          var elementName = PsiTreeUtil.findChildOfAnyType(elementDefinition, LuaClassMethodName.class, LuaClassVarName.class);
